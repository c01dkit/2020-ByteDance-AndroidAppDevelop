package com.bytedance.camp.chapter_4.widget;
...
public class ClockView extends View {
    ...
    private static final float NUMBER_TEXT_WIDTH = 65f; // 数字字体大小
    private static final float NUMBER_TEXT_POSITION_RATIO = 0.87f; // 数字位置相对于表盘半径的比例
    ...
    private void init() {
        ...
        numberPaint.setTextSize(NUMBER_TEXT_WIDTH);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        Log.i(TAG, "onLayout: layout re-config");
    }
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        // 绘制分针：一分钟贡献6°，一秒钟贡献0.1°，减去偏转度数90
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        double minuteRadian = Math.toRadians(minute * 6 + second * 0.1f - 90);
        endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(minuteRadian));
        endY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(minuteRadian));
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);

        // 绘制秒针：一秒钟贡献6°，减去偏转度数90
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        needlePaint.setColor(Color.RED);
        double secondRadian = Math.toRadians(second * 6 - 90);
        endX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(secondRadian));
        endY = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(secondRadian));
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);
        needlePaint.setColor(Color.WHITE);

        // 绘制时针：一小时贡献30°，一分钟贡献0.5°，一秒钟贡献0.0083°，减去偏转度数90
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        double hourRadian = Math.toRadians(hour * 30 + minute * 0.5f + second * 0.0083f - 90);
        endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(hourRadian));
        endY = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(hourRadian));
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);
    ...
    private void drawTimeNumbers(Canvas canvas) {
        // TODO 绘制表盘时间数字（可选）
        float textX, textY, baseLine;
        double radian;
        Paint.FontMetrics fontMetrics = numberPaint.getFontMetrics();
        for (int i = 1; i <= 12; i++) {
            radian = Math.toRadians(i * 30 - 90);
            textX = (float) (centerX + radius * NUMBER_TEXT_POSITION_RATIO * Math.cos(radian));
            textY = (float) (centerY + radius * NUMBER_TEXT_POSITION_RATIO * Math.sin(radian));
            baseLine = textY + (fontMetrics.bottom - fontMetrics.top)/2 -fontMetrics.bottom;
            canvas.drawText(Integer.toString(i),textX,baseLine,numberPaint);
        }
    }
